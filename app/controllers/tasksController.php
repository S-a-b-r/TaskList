...
if ($method == 'POST') {
    switch ($_POST['action']) {
        case 'create': return create($_REQUEST['description']);
        case 'remove_all': return removeAll();
        case 'ready_all': return readyAll();
        case 'ready': return ready($_REQUEST['task_id']);
        case 'unready': return unready($_REQUEST['task_id']);
        case 'remove': return remove($_REQUEST['task_id']);
    }
    return new View('error', ['message'=>'Страница не найдена']);
}
